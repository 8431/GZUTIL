...
import com.gz.medicine.common.mybatisPageVo.ReflectHelperUtil;
...
public class PageInterceptor implements Interceptor {
    ...
    public Object intercept(Invocation invocation) throws Throwable {
        ...
        //这里我们简单的通过传入的是Page对象就认定它是需要进行分页操作的。
            ...
            if(id.matches(".+queryPage.*")&&obj instanceof Page<?>){
                Page<?> page = (Page<?>) obj;
                Connection connection = (Connection) invocation.getArgs()[0];
                //设置总页数
                setTotalRecord( page,mappedStatement,connection);
                //拼凑自定义sql
                String pageSql=getPageSql(page,boundSql.getSql());
                //利用反射设置当前BoundSql对应的sql属性为我们建立好的分页Sql语句
                ReflectHelperUtil.setFieldValue(boundSql, "sql", pageSql);
            }
